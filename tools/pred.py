...
def initialize(config, beam_width=config.BEAM_WIDTH):
    ...
    global charset, decodes, shape, indices, values, prob, inputdata, batch_size
    ...
    with g.as_default():
        ...
        decodes, shape, indices, values, prob, inputdata, batch_size = build_graph(g, charset, beam_width)
...
def build_graph(g, charset, beam_width=config.BEAM_WIDTH):
    with g.as_default():
        ...
        #                                           sequence_length=batch_size,
        #                                           merge_repeated=False)
        shape = decodes[0].dense_shape
        indices = decodes[0].indices
        values = decodes[0].values
        return decodes[0], shape, indices, values, prob, inputdata, batch_size
    ...
    for i in range(0, len(image_list), _batch_size):
        ...
        with graph.as_default():
            with sess.as_default():
                _shape, _indices, _values, _prob = sess.run(
                    [shape, indices, values, prob],
                    feed_dict={
                        inputdata: _input_data,
                        batch_size: _batch_size_array  # 这个是为了指导LSTM，以及CTC Beam Seach的参数
                    })
                preds = tf.SparseTensor(_indices, _values, _shape)
                # 将结果，从张量变成字符串数组，session.run(arg)arg是啥类型，就ruturn啥类型
                preds = data_utils.sparse_tensor_to_str_new(preds, charset)
