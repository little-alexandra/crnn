...
def binary_img_segment(binary_img,original_img=None):
    ...
    for contour in contours:
        ...
        x, y, w, h = location
        if(w*h<100):
            print("w*h<100，忽略: %d*%d=%d" %(w,h,w*h))
            continue
        ...
        if original_img:#  len(original_img)
...
if __name__ == '__main__':
    ...
    img = cv2.Canny(img, 128, 256)
    # print(img)
    import sys
    np.set_printoptions(threshold=sys.maxsize)

    _,img = read_img_and_convert_to_binary("0.png")

    cv2.imshow("img",img)
    symbols = binary_img_segment(img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    print("最后返回%d个区域" % len(symbols))
