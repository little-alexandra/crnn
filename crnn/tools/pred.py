...
from crnn.local_utils import log_utils, data_utils, image_util
...
def build_graph(g, charset, beam_width=config.BEAM_WIDTH):
    with g.as_default():
        ...
        with tf.variable_scope('shadow'):
            net_out,raw_pred = net.build(inputdata=inputdata, sequence_len=batch_size)
...
def pred(params,image_list, _batch_size,tf_serving_call=None):
        ...
        if tf_serving_call is None:
            ...
            with graph.as_default():
                with sess.as_default():
                    _decodes, _prob = sess.run(
                        [decodes, prob],
                        feed_dict={
                            inputdata: _input_data,
                            batch_size: _batch_size_array  # 这个是为了指导LSTM，以及CTC Beam Seach的参数
                        })
                    indices, values, dense_shape = _decodes.indices,_decodes.values,_decodes.dense_shape
        else:
            indices, values, dense_shape = tf_serving_call(_input_data, _batch_size_array)

        # 将结果，从张量变成字符串数组，session.run(arg)arg是啥类型，就ruturn啥类型
        preds = data_utils.sparse_tensor_to_str(indices, values, dense_shape, charset)
